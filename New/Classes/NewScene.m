 ...
 *  New
 *
 *  Created by Christian M. Mata on 1/17/13.
...
-(void) initializeScene {
	...
	cam.location = cc3v( 4.0, 0.5, 55.0 );
	[self addChild: cam];
	...
	CC3MeshNode* aNode = [[CC3BoxNode alloc]init];
	CC3BoundingBox bBox;
	bBox.minimum = cc3v(-5.0, -5.0, -5.0);
	bBox.maximum = cc3v( 10.0,  10.0,  10.0);
	[aNode populateAsSolidBox: bBox];
	aNode.material = [CC3Material material];
	aNode.color = ccRED;
    aNode.location = cc3v(0.0, 0.0, 0.0);
    //Messing with these numbers to rotate the cube
   // [aNode rotateBy:CC3VectorMake(40, -20, -69)];
    
    [self addChild:aNode];
    
    CCActionInterval* partialRot = [CC3RotateBy actionWithDuration: 0.1
														  rotateBy: cc3v(20.0, 0.0, 0.0)];
