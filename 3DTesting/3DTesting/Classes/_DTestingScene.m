...
CC3Node *testBox, *tBox;


@implementation _DTestingScene
...
-(void) initializeScene {
    ...
    CC3PODResourceNode *testBox = [CC3PODResourceNode nodeWithName:@"Box"];
    testBox.resource = [CC3PODResource resourceFromFile:@"test_Cube.pod"];
    
    [testBox setAmbientColor:kCCC4FWhite];
    [testBox addAxesDirectionMarkers];
    [self addChild:testBox];
    
    [testBox setScale:cc3v(1, 1, 1)];
    [testBox setLocation:cc3v(0, 0, 0)];
    [testBox setRotation:cc3v(-90, 0, 0)];
    [testBox enableAllAnimation];
    
    CC3PODResourceNode *tBox = [CC3PODResourceNode nodeWithName:@"Box"];
    tBox.resource = [CC3PODResource resourceFromFile:@"test_Cube.pod"];
    
    [tBox setAmbientColor:kCCC4FWhite];
    [tBox addAxesDirectionMarkers];
    [self addChild:tBox];
    
    [testBox setScale:cc3v(1, 1, 1)];
    [testBox setLocation:cc3v(0, 0, .1)];
    [testBox setRotation:cc3v(-90, 0, 0)];
    [testBox enableAllAnimation];
...
//    //Animate box
//    CC3MeshNode* testCube = (CC3MeshNode*)[self getNodeNamed: @"Box"];
//    CCActionInterval* partialRotC = [CC3RotateBy actionWithDuration: 1.0
//														  rotateBy: cc3v(0.0, 10.0, 0.0)];
//	[testCube runAction: [CCRepeatForever actionWithAction: partialRotC]];
...
-(void) updateAfterTransform: (CC3NodeUpdatingVisitor*) visitor {
    [self checkCollisions:tBox :testBox];
}

-(void) checkCollisions: (CC3Node*)first: (CC3Node*) second
{
    if([second doesIntersectBoundingVolume: first.boundingVolume])
    {
        NSLog(@"BOOM! %@ hit %@", first, second);
        [self removeChild: second];
        CCActionInterval* moveUp = [CC3MoveForwardBy actionWithDuration:10.0
                                                                 moveBy:50.0];
        [first runAction: moveUp];
    }
    
    else{
        NSLog(@"Crap: We're here!");
    }
}
...
- (void)handleGesture: (CGPoint) aVelocity
{
    ...
    if(aVelocity.x > 0 && aVelocity.x > aVelocity.y)
    {
        NSLog(@"gesture went right");
        CC3MeshNode* testCube = (CC3MeshNode*)[self getNodeNamed: @"Box"];
        CCActionInterval* partialRotC = [CC3RotateBy actionWithDuration: .5 rotateBy: cc3v(0.0, 90.0, 0.0)];
        [testCube runAction: partialRotC];
        [self updateTransformMatrices];
    }
    else if (aVelocity.x < 0 && aVelocity.x < aVelocity.y)
    {
    ...
    else if (aVelocity.y > 0 && aVelocity.x < aVelocity.y)
    {
        
        NSLog(@"gesture went up");
        CC3MeshNode* testCube = (CC3MeshNode*)[self getNodeNamed: @"Box"];
        CCActionInterval* partialRotC = [CC3RotateBy actionWithDuration: .5 rotateBy: cc3v(90.0, 0.0, 0.0)];
        [testCube runAction: partialRotC];
        [self updateTransformMatrices];
    }
    else
